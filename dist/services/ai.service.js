...
exports.processSowWithAi = processSowWithAi;
...
async function processSowWithAi(sowText, projectId) {
...
`;
    ...
    for (const secData of parsed.sections) {
        const sectionId = (0, crypto_1.randomUUID)();
        await db_1.db.insert(schema_1.section).values({
            id: sectionId,
            projectId,
            title: secData.title,
            order: sectionOrder++,
        ...
        if (secData.cards && Array.isArray(secData.cards)) {
            for (const cardData of secData.cards) {
                ...
                await db_1.db.insert(schema_1.scopeCard).values({
                    id: cardId,
                    projectId,
                    sectionId,
    ...
    // Add activity log
    await db_1.db.insert(schema_1.activityLog).values({
