import { TRPCError } from "@trpc/server";
import { and, eq } from "drizzle-orm";
import { randomUUID } from "crypto";
import crypto from "crypto";
import { z } from "zod";
import { activityLog, magicLink, project } from "../db/schema";
import { protectedProcedure, router } from "../trpc";

export const magicLinkRouter = router({
  generate: protectedProcedure
    .input(
      z.object({
        projectId: z.string(),
        expiresInDays: z.number().default(90),
      })
    )
    .mutation(async ({ input, ctx }) => {
      const { agencyId } = ctx.user;

      const proj = await ctx.db.query.project.findFirst({
        where: (p, { and, eq }) => and(eq(p.id, input.projectId), eq(p.agencyId, agencyId)),
      });

      if (!proj) {
        throw new TRPCError({
          code: "NOT_FOUND",
          message: "Project not found.",
        });
      }

      // Deactivate any existing magic links for this project
      await ctx.db
        .update(magicLink)
        .set({ isActive: false, updatedAt: new Date().toISOString() })
        .where(and(eq(magicLink.projectId, input.projectId), eq(magicLink.isActive, true)));

      const chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
      let token = "";
      const bytes = crypto.randomBytes(8);
      for (let i = 0; i < 8; i++) {
        token += chars[bytes[i] % chars.length];
      }
      const expiresAt = new Date();
      expiresAt.setDate(expiresAt.getDate() + input.expiresInDays);
      const now = new Date().toISOString();
      const id = randomUUID();

      await ctx.db.insert(magicLink).values({
        id,
        projectId: input.projectId,
        token,
        expiresAt: expiresAt.toISOString(),
        isActive: true,
        createdAt: now,
        updatedAt: now,
      });

      // Update project status to SENT if it was DRAFT
      if (proj.status === "DRAFT") {
        await ctx.db
          .update(project)
          .set({ status: "SENT", updatedAt: now })
          .where(eq(project.id, input.projectId));
      }

      await ctx.db.insert(activityLog).values({
        id: randomUUID(),
        projectId: proj.id,
        action: "MAGIC_LINK_GENERATED",
        details: JSON.stringify({ token, expiresAt: expiresAt.toISOString() }),
        createdAt: now,
      });

      const result = await ctx.db.query.magicLink.findFirst({
        where: (m, { eq }) => eq(m.id, id),
      });
      return result!;
    }),

  revoke: protectedProcedure
    .input(z.object({ id: z.string() }))
    .mutation(async ({ input, ctx }) => {
      const link = await ctx.db.query.magicLink.findFirst({
        where: (m, { eq }) => eq(m.id, input.id),
        with: { project: true },
      });

      if (!link || link.project.agencyId !== ctx.user.agencyId) {
        throw new TRPCError({
          code: "NOT_FOUND",
          message: "Magic link not found.",
        });
      }

      await ctx.db
        .update(magicLink)
        .set({ isActive: false, updatedAt: new Date().toISOString() })
        .where(eq(magicLink.id, input.id));

      const updated = await ctx.db.query.magicLink.findFirst({
        where: (m, { eq }) => eq(m.id, input.id),
      });
      return updated!;
    }),
});
