...
export const projectRouter = router({
  create: protectedProcedure
    .input(
      z.object({
        ...
        type: z.enum(["SOFTWARE", "CREATIVE", "ARCHITECTURE", "CONSULTING", "OTHER"]).default("SOFTWARE"),
  ...
  list: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      let conditions = [eq(project.agencyId, agencyId)];

      if (input.status && input.status !== "ALL") {
        conditions.push(eq(project.status, input.status));
      }
      if (input.type && input.type !== "ALL") {
        conditions.push(eq(project.type, input.type));
      }
  ...
  update: protectedProcedure
    .input(
      z.object({
        ...
        clientCompany: z.string().optional(),
        clientWhatsApp: z.string().optional(),
        type: z.enum(["SOFTWARE", "CREATIVE", "ARCHITECTURE", "CONSULTING", "OTHER"]).optional(),
        currency: z.string().optional(),
        status: z.enum(["DRAFT", "SENT", "IN_REVIEW", "APPROVED", "SIGNED", "ARCHIVED"]).optional(),
      })
    )
    .mutation(async ({ input, ctx }) => {
      const { id, ...data } = input;
  ...
  getStats: protectedProcedure.query(async ({ ctx }) => {
    ...
    const pendingApprovalsCount = projects.filter(
      (p) => p.status === "IN_REVIEW" || p.status === "SENT"
    ).length;

    let totalChangeRequests = 0;
    projects.forEach((p) => {
      totalChangeRequests += p.changeRequests.filter(
        (cr) => cr.status === "NEW" || cr.status === "PRICED"
      ).length;
    });
